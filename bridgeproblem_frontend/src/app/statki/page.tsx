...
const Home: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const waiting = shipsData.filter(
        (ship: { state: string }) => ship.state === "WAITING"
      );
      const processing = shipsData.filter(
        (ship: { state: string }) => ship.state === "PROCESSING"
      );
      const processed = shipsData.filter(
        (ship: { state: string }) => ship.state === "PROCESSED"
      );
  ...
  const generateShips = async () => {
    try {
      await fetch("http://localhost:8080/api/generate-ships", {
        method: "GET",
      });
      fetchData();
    } catch (error) {
  ...
  return (
    <div>
      ...
      <div className="text-red-600">dsadsa</div>
    </div>
  );
};
